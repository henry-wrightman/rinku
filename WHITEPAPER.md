...
```json
{
  ...
  "hash": "sha256(tx)",
  "tipUrls": ["hash1", "hash2"],  // On-chain tx parent references
  "parents": [/* expanded proof bundles for tipUrls */],
  "checkpoint": {
    "id": "cp_789",
    "merkleRoot": "...",
    "height": 1000,
...
> **Note:** BLS signature fields (`blsAggregateSig`, `signerBitmap`) may be included for convenience. However, without `validatorProof`, offline verification requires a trusted validator set or trusted checkpoint feed.
...
interface ProofBundle {
  ...
  tipUrls: string[];      // On-chain parent tx hashes (from tx.tipUrls)
  parents: ProofBundle[]; // Expanded proof bundles for tipUrls
  checkpoint: {
    id: string;
    merkleRoot: string;
    height: number;
...
### A.5 Merkle Tree Semantics

Rinku uses binary Merkle trees with the following semantics:

- **merkleIndex:** 0-based leaf position in the ordered tree
- **Leaf ordering:** Transactions are ordered by their inclusion order in the checkpoint; state proofs use lexicographically sorted account addresses
- **Padding:** Natural binary tree with duplicate-last-leaf padding when leaf count is odd (`right = leaves[i+1] || left`)
- **Numeric precision:** Balance and stake values use u64 micro-units (1 RKU = 100,000,000 micro-RKU) for deterministic cross-language verification

**Account State Proof encoding (for SDK implementers):**
- Leaf encoding: `SHA256("account:{address}:{balance_micro}:{nonce}:{staked_micro}")`
- Internal node encoding: `SHA256("node:{left_hash}:{right_hash}")`

**Transaction Merkle Proof encoding:**
- Leaf: raw transaction hash (hex)
- Internal node: `SHA256(left_hash + right_hash)` (concatenated hex strings)

### A.6 tipUrls vs parents

- `tipUrls`: On-chain transaction parent references (hash pointers to 1-2 tip transactions in the DAG)
- `parents`: Expanded proof bundles for those references, containing the complete verification data needed for recursive validation
