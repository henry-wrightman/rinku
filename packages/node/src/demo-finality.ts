...
interface DAGResponse {
  nodes: {
    tx: { hash: string; from: string; to: string; amount: number; ts: number };
  }[];
}
...
async function main() {
  ...
  console.log(
    `   Genesis Time: ${new Date(genesisConfig.genesisTime).toISOString()}`,
  );
  console.log(`   Genesis Checkpoint: ${genesisConfig.genesisCheckpointId}`);
  console.log(
    `   Initial Validators: ${genesisConfig.initialValidators.length}\n`,
  );
  ...
  console.log(
    `   Checkpoint created: ${createData.checkpoint?.checkpointId || "already exists"}\n`,
  );
  ...
  console.log(`   Amount: ${sampleTx.tx.amount} RKU\n`);

  console.log("4. Getting finalized URL with embedded proof...");
  const finalizedRes = await fetch(
    `${NODE_URL}/api/tx/${sampleTx.tx.hash}/finalized`,
  );
  ...
  console.log(
    `   Proof overhead: ${finalizedData.finalizedUrl!.length - finalizedData.txUrl!.length} chars\n`,
  );
  ...
  console.log(
    `   - Previous Checkpoint: ${proof.previousCheckpointId || "genesis"}`,
  );
  ...
  console.log(
    "   (Verifying proof using ONLY genesis config as trust anchor)\n",
  );
  ...
  if (proof.checkpointHeight === 0 || proof.validators.length === 0) {
    ...
    console.log(
      `   Verified Weight: ${verification.validatorWeightPercent.toFixed(1)}%`,
    );
