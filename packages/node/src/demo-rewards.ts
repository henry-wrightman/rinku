...
async function main() {
  ...
  try {
    ...
    if (faucetData.success) {
      ...
      console.log(`   Balance: ${faucetData.amount} coins`);
    } else {
      ...
      const account = accountsData.accounts.find(a => a.balance >= 200 && a.fingerprint !== 'faucet');
      if (account) {
        testAddress = account.fingerprint;
        console.log(`   Using existing wallet: ${testAddress.slice(0, 16)}... with ${account.balance} coins`);
      } else {
  ...
  if (finalStaking.topStakers.length > 0) {
    ...
    for (const staker of finalStaking.topStakers.slice(0, 5)) {
      console.log(`     - ${staker.staker.slice(0, 12)}... : ${staker.amount} coins`);
    }
  }

  console.log('\n=== Demo Complete ===');
  console.log('\nVisit the explorer and click "rewards" tab to see your staking status!');
  console.log('Rewards are automatically distributed when transactions are processed.');
}
