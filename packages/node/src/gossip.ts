...
import { PeerSyncService, PeerInfo } from "./peer-sync.js";
...
export class GossipService {
  ...
  private validatorSet: Map<
    string,
    { publicKey: Uint8Array; weight: number; registeredAt: number }
  > = new Map();
  private validatorSetMerkle: string = "";

  private gossipInterval: NodeJS.Timeout | null = null;
  private onTxReceived:
    | ((tx: SignedTransaction, publicKey?: Uint8Array) => Promise<boolean>)
    | null = null;
  private onCheckpointQuorum:
    | ((
        checkpointId: string,
        signatures: Map<
          string,
          { signature: string; publicKey: Uint8Array; weight: number }
        >,
      ) => Promise<void>)
    | null = null;
  ...
  setTxReceivedCallback(
    cb: (tx: SignedTransaction, publicKey?: Uint8Array) => Promise<boolean>,
  ): void {
    this.onTxReceived = cb;
  }

  setCheckpointQuorumCallback(
    cb: (
      checkpointId: string,
      signatures: Map<
        string,
  ...
  start(): void {
    ...
    console.log(
      `Gossip service started (interval: ${this.config.gossipIntervalMs}ms)`,
    );
  }
  ...
  private computeValidatorSetMerkle(): string {
    const sorted = Array.from(this.validatorSet.entries()).sort((a, b) =>
  ...
  detectConflict(
    ...
    console.log(
      `Conflict detected: ${conflictId} for account ${account.slice(0, 16)}...`,
    );
  ...
  private checkConflictResolution(conflict: ConflictRecord): void {
    ...
    const totalValidatorWeight =
      Array.from(this.validatorSet.values()).reduce(
        (sum, v) => sum + v.weight,
        0,
      ) || 1;
    ...
    for (const [txHash, weight] of voteWeights) {
      if (
        weight / totalValidatorWeight >=
        this.config.conflictResolutionThreshold
      ) {
        const loserHash =
          txHash === conflict.txHash1 ? conflict.txHash2 : conflict.txHash1;
  ...
  private async resolveConflict(
    ...
    console.log(
      `Conflict ${conflict.conflictId} resolved: winner=${winnerHash.slice(0, 16)}...`,
    );
  ...
  private async gossipRound(): Promise<void> {
    ...
    const txBatch = Array.from(this.pendingTxs.values()).slice(
      0,
      this.config.txBatchSize,
    );
  ...
  private async sendTxBatch(
    peer: PeerInfo,
    txBatch: TxGossipPayload[],
  ): Promise<void> {
  ...
  private async sendValidatorSet(peer: PeerInfo): Promise<void> {
    const validators = Array.from(this.validatorSet.entries()).map(
      ([addr, v]) => ({
        address: addr,
  ...
  async receiveGossipMessage(message: GossipMessage): Promise<void> {
    ...
    switch (message.type) {
      case "tx":
        ...
        break;
      case "tip_announce":
        await this.receiveTipAnnounce(
          message.payload as TipAnnouncePayload,
          message.nodeId,
        );
        break;
      case "checkpoint_sig":
        this.receiveCheckpointSignature(
          message.payload as CheckpointSigPayload,
        );
        break;
      case "validator_set":
      ...
      case "conflict_resolution":
        await this.receiveConflictResolution(
  ...
  private async receiveTipAnnounce(
    payload: TipAnnouncePayload,
    fromNodeId: string,
  ): Promise<void> {
    if (payload.tips.length > GossipService.MAX_HEALTHY_TIPS) {
      ...
      if (Date.now() - lastWarn > GossipService.TIP_WARN_INTERVAL_MS) {
        console.warn(
          `Ignoring unhealthy peer ${fromNodeId.slice(0, 8)} with ${payload.tips.length} tips (max: ${GossipService.MAX_HEALTHY_TIPS})`,
        );
    ...
    const missingTips = payload.tips.filter(
      (tip) => !ourTips.has(tip) && !this.consensus.hasTransaction(tip),
    );
    ...
    if (missingTips.length > 0) {
      ...
      if (Date.now() - lastWarn > GossipService.TIP_WARN_INTERVAL_MS) {
        console.log(
          `Node ${fromNodeId.slice(0, 8)} has ${missingTips.length} tips we're missing`,
        );
        this.lastTipWarnTime.set(fromNodeId, Date.now());
      }
    }
  ...
  private receiveCheckpointSignature(payload: CheckpointSigPayload): void {
    ...
    if (!pending) {
      ...
      pending = {
        ...
        totalWeight: 0,
        requiredWeight:
          totalWeight * (this.config.checkpointQuorumPercent / 100),
      };
    ...
    if (!pending.signatures.has(payload.validatorAddress)) {
      ...
      console.log(
        `Received checkpoint signature from ${payload.validatorAddress.slice(0, 16)}... (${pending.totalWeight}/${pending.requiredWeight} weight)`,
      );

      if (
        pending.totalWeight >= pending.requiredWeight &&
        this.onCheckpointQuorum
      ) {
  ...
  private receiveValidatorSet(payload: ValidatorSetPayload): void {
    if (payload.merkleRoot !== this.validatorSetMerkle) {
      console.log(
        `Validator set mismatch: local=${this.validatorSetMerkle}, remote=${payload.merkleRoot}`,
      );
    }
  }

  private async receiveConflictResolution(
    ...
    if (this.onConflictResolved) {
      const loserHash =
        payload.winnerHash === payload.txHash1
          ? payload.txHash2
          : payload.txHash1;
  ...
  getStats(): {
    ...
    return {
      ...
      activeConflicts: Array.from(this.conflicts.values()).filter(
        (c) => !c.resolvedAt,
      ).length,
  ...
  toJSON(): object {
    return {
      ...
      pendingCheckpointSigs: Array.from(
        this.pendingCheckpointSigs.entries(),
      ).map(([id, pending]) => ({
        checkpointId: id,
        height: pending.height,
        signatures: Array.from(pending.signatures.entries()).map(
          ([addr, sig]) => ({
      ...
      validatorSet: Array.from(this.validatorSet.entries()).map(
        ([addr, v]) => ({
          address: addr,
          publicKey: Array.from(v.publicKey),
          weight: v.weight,
  ...
  static fromJSON(
    ...
    if (data.validatorSet) {
      ...
      service.validatorSetMerkle =
        data.validatorSetMerkle || service.computeValidatorSetMerkle();
    }
