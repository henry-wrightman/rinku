...
async function main() {
  ...
  for (let i = 0; i < WALLET_COUNT; i++) {
    wallets.push(
      `wallet_${i.toString().padStart(3, "0")}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`,
    );
  }
  ...
  if (tipUrls.length > 0) {
    ...
    while (queue.length > 0) {
      ...
      if (transactions.length % 5 === 0) {
        process.stdout.write(
          `\r  Crawled: ${transactions.length} transactions, depth: ${depth}, queue: ${queue.length}  `,
        );
      }
    }
    ...
    console.log(
      `\r  Crawled: ${transactions.length} transactions in ${crawlElapsed}s          `,
    );
    ...
    if (sampleAccounts.length > 0) {
      ...
      for (const [fp, balance] of sampleAccounts) {
        console.log(`    ${fp.slice(0, 25)}... : ${balance} RKU`);
      }
    }
    ...
    for (const tx of sampleTxs) {
      const shortHash =
        typeof tx.hash === "string" ? tx.hash.slice(0, 8) : "n/a";
      const parents = tx.tipUrls
        .map((u) => {
      ...
      console.log(
        `  ${shortHash}: ${fromStr}→${toStr} refs [${parents || "genesis"}]`,
      );
    }
  ...
  console.log("-".repeat(40));
  console.log(
    `  Total transactions in DAG: ${dag.nodes?.length || dag.transactions?.length || 0}`,
  );
