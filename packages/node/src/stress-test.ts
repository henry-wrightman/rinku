...
async function generateTransactions(count: number): Promise<number> {
  ...
  console.log("(Each faucet tx creates a new account with 100 RKU)\n");
  ...
  for (let i = 0; i < count; i++) {
    ...
    if ((i + 1) % 10 === 0 || i === count - 1) {
      ...
      const tps =
        success > 0 ? (success / parseFloat(elapsed)).toFixed(1) : "0";
      process.stdout.write(
        `\r  Progress: ${success + failed}/${count} (${failed} failed) - ${elapsed}s elapsed - ${tps} tx/s`,
      );
    }
  }
...
async function validateState(): Promise<void> {
  ...
  console.log(`  Total RKU: ${totalBalance}`);
}
...
async function testBootstrapSync(peerUrl: string): Promise<boolean> {
  ...
  console.log(
    `  Source node: ${sourceStatus.dagSize} transactions, root: ${sourceStatus.merkleRoot.slice(0, 16)}...`,
  );
