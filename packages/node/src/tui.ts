...
import type { PeerSyncService } from "./peer-sync.js";
...
export class NodeTui {
  ...
  private renderDashboard(): void {
    ...
    const protoVer = this.deps.protocolVersion
      ? ` (proto ${this.deps.protocolVersion})`
      : "";
    ...
    process.stdout.write(
      `${COLORS.cyan}║${COLORS.reset}${header}${" ".repeat(width - 30 - uptime.length + 20)}${uptime}${COLORS.cyan}║${COLORS.reset}\n`,
    );
    process.stdout.write(`${COLORS.cyan}╠${line("═")}╣${COLORS.reset}\n`);
    ...
    const rows = [
      [
        `${COLORS.green}DAG${COLORS.reset}`,
        `${COLORS.magenta}SYSTEM${COLORS.reset}`,
      ],
      [
        `├─ Nodes: ${dag.getAllNodes().length}`,
        `├─ CPU: ${telemetry.cpu.usage}%`,
      ],
      [
        `├─ Tips: ${tips.length}`,
        `├─ Heap: ${formatBytes(telemetry.memory.heapUsed)}`,
      ],
      [
        `├─ Accounts: ${accountCount}`,
        `├─ RSS: ${formatBytes(telemetry.memory.rss)}`,
      ],
      [
        `└─ Witnessed: ${witnessedCount}`,
        `└─ Data: ${formatBytes(telemetry.disk.dataDir)}`,
      ],
      ["", ""],
      [
        `${COLORS.yellow}CONSENSUS${COLORS.reset}`,
        `${COLORS.blue}NETWORK${COLORS.reset}`,
      ],
      [
        `├─ Checkpoint: #${checkpointHeight}`,
        `├─ Peers: ${peerCount}/${maxPeers}`,
      ],
      [
        `├─ Pending: ${pendingCount} txs`,
        `├─ In: ${formatBytes(telemetry.network.rateIn)}/s`,
      ],
      [
        `├─ Gas: ${gasPrice.toFixed(4)} RKU`,
        `├─ Out: ${formatBytes(telemetry.network.rateOut)}/s`,
      ],
      [
        `└─ TPS: ${tps}`,
        `└─ Gossip: ${this.deps.gossip ? "active" : "disabled"}`,
      ],
    ];
    ...
    for (const [left, right] of rows) {
      ...
      process.stdout.write(
        `${COLORS.cyan}║${COLORS.reset} ${left}${" ".repeat(Math.max(0, leftPad))}│ ${right}${" ".repeat(Math.max(0, rightPad))}${COLORS.cyan}║${COLORS.reset}\n`,
      );
    }
    ...
    process.stdout.write(
      `${COLORS.cyan}║${COLORS.reset} ${menuItems}${" ".repeat(Math.max(0, menuPad))}${COLORS.cyan}║${COLORS.reset}\n`,
    );
    process.stdout.write(`${COLORS.cyan}╚${line("═")}╝${COLORS.reset}\n`);
  }
  ...
  private renderLogs(): void {
    ...
    process.stdout.write(
      `${COLORS.cyan}${COLORS.bold}═══ LIVE LOGS ═══${COLORS.reset}  ${COLORS.dim}[ESC] Back to Dashboard${COLORS.reset}\n\n`,
    );
  ...
  private renderPeers(): void {
    this.clearScreen();
    process.stdout.write(
      `${COLORS.cyan}${COLORS.bold}═══ PEER CONNECTIONS ═══${COLORS.reset}  ${COLORS.dim}[ESC] Back to Dashboard${COLORS.reset}\n\n`,
    );
    ...
    process.stdout.write(
      `${COLORS.bold}  ID              URL                          Status${COLORS.reset}\n`,
    );
    ...
    for (const peer of peers) {
      ...
      process.stdout.write(
        `  ${id.padEnd(14)}  ${url.padEnd(30)}  ${statusIcon}\n`,
      );
    }
  ...
  private renderDag(): void {
    this.clearScreen();
    process.stdout.write(
      `${COLORS.cyan}${COLORS.bold}═══ DAG DETAILS ═══${COLORS.reset}  ${COLORS.dim}[ESC] Back to Dashboard${COLORS.reset}\n\n`,
    );
    ...
    process.stdout.write(
      `${COLORS.bold}Total Nodes:${COLORS.reset} ${nodes.length}\n`,
    );
    process.stdout.write(
      `${COLORS.bold}Active Tips:${COLORS.reset} ${tips.length}\n\n`,
    );
    ...
    for (const tipHash of tips.slice(0, 10)) {
      ...
      if (node) {
        ...
        process.stdout.write(
          `  ${COLORS.dim}${tipHash.slice(0, 16)}...${COLORS.reset} from ${sender} w=${weight}\n`,
        );
      }
    }
    ...
    if (tips.length > 10) {
      process.stdout.write(
        `  ${COLORS.dim}... and ${tips.length - 10} more${COLORS.reset}\n`,
      );
    }
  }
  ...
  private renderSpecs(): void {
    this.clearScreen();
    process.stdout.write(
      `${COLORS.cyan}${COLORS.bold}═══ SYSTEM SPECIFICATIONS ═══${COLORS.reset}  ${COLORS.dim}[ESC] Back to Dashboard${COLORS.reset}\n\n`,
    );
    ...
    process.stdout.write(
      `  Heap Used:    ${formatBytes(telemetry.memory.heapUsed)} / ${formatBytes(telemetry.memory.heapTotal)}\n`,
    );
    process.stdout.write(
      `  RSS Memory:   ${formatBytes(telemetry.memory.rss)}\n`,
    );
    process.stdout.write(
      `  System Free:  ${formatBytes(telemetry.memory.systemFree)}\n`,
    );
    process.stdout.write(
      `  Uptime:       ${formatDuration(telemetry.processUptime)}\n\n`,
    );

    process.stdout.write(`${COLORS.bold}Network${COLORS.reset}\n`);
    process.stdout.write(
      `  Total In:     ${formatBytes(telemetry.network.bytesIn)}\n`,
    );
    process.stdout.write(
      `  Total Out:    ${formatBytes(telemetry.network.bytesOut)}\n`,
    );
    process.stdout.write(
      `  Data Dir:     ${formatBytes(telemetry.disk.dataDir)}\n`,
    );
  }
  ...
  private renderThreads(): void {
    this.clearScreen();
    process.stdout.write(
      `${COLORS.cyan}${COLORS.bold}═══ THREAD CONFIGURATION ═══${COLORS.reset}  ${COLORS.dim}[ESC] Back to Dashboard${COLORS.reset}\n\n`,
    );
    ...
    process.stdout.write(
      `${COLORS.bold}Crypto Worker Threads${COLORS.reset}\n\n`,
    );
    ...
    process.stdout.write(
      `${COLORS.dim}Use ↑/↓ arrows to adjust thread count${COLORS.reset}\n\n`,
    );

    process.stdout.write(`${COLORS.yellow}Recommendations:${COLORS.reset}\n`);
    process.stdout.write(`  • Low traffic:     1-2 threads\n`);
    process.stdout.write(
      `  • Medium traffic:  ${Math.max(2, Math.floor(maxWorkers / 2))} threads\n`,
    );
    process.stdout.write(`  • High traffic:    ${maxWorkers - 1} threads\n`);
    process.stdout.write(`  • Validator node:  ${maxWorkers} threads\n\n`);

    process.stdout.write(
      `${COLORS.dim}Note: Changes take effect immediately for new operations.${COLORS.reset}\n`,
    );
    process.stdout.write(
      `${COLORS.dim}Set CRYPTO_WORKERS env var to persist across restarts.${COLORS.reset}\n`,
    );
  }
}
