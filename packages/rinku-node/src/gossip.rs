...
impl GossipService {
    ...
    pub async fn handle_message(&self, message: GossipMessage) -> Result<Option<GossipMessage>> {
        ...
        match message {
            ...
            GossipMessage::CheckpointAnnouncement { checkpoint, finalized_tx_hashes, precomputed_proofs, .. } => {
                ...
                if checkpoint.height == local_height + 1 {
                    info!(
                        "Received checkpoint announcement {} at height {} (leader broadcast, {} finalized txs, {} proofs)",
                        &checkpoint.hash[..16.min(checkpoint.hash.len())],
                        checkpoint.height,
                        finalized_tx_hashes.len(),
                    ...
                    if let Err(e) = self.state.apply_checkpoint_with_finalized_hashes(
                        ...
                        // Store precomputed proofs received from the leader
                        // This ensures followers have valid proofs without regenerating them
