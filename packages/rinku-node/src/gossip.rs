...
pub enum GossipMessage {
    ...
    ViewChange {
        ...
        validator_address: String,
        validator_stake: u64,
        reason: rinku_core::types::ViewChangeReason,
        #[serde(default, skip_serializing_if = "Option::is_none")]
        bls_signature: Option<String>,
        timestamp_ms: u64,
        #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl GossipMessage {
    pub fn variant_name(&self) -> &'static str {
        match self {
            ...
            GossipMessage::ViewChange { .. } => "ViewChange",
...
pub struct GossipServiceInner {
    ...
    pub view_change_votes: HashMap<(u64, u32), HashMap<String, rinku_core::types::ViewChangeVote>>,
    pub view_change_sent: std::collections::HashSet<(u64, u32)>,
    pub current_view: HashMap<u64, u32>,
    pub push_retry_queue: Vec<PendingCheckpointPush>,
}

pub struct PendingCheckpointPush {
    pub peer_id: String,
    pub push_data: std::sync::Arc<crate::network::CheckpointPushData>,
    pub attempts: u8,
...
impl GossipService {
    pub fn new(
        ...
        Self {
            ...
            inner: Arc::new(RwLock::new(GossipServiceInner {
                ...
                view_change_sent: std::collections::HashSet::new(),
                current_view: HashMap::new(),
                push_retry_queue: Vec::new(),
            })),
            node_id,
            interval_ms,
    ...
    async fn run_sync_request_handler(
        ...
        info!("P2P sync request handler started (concurrent, max 2 delta syncs)");
        let delta_semaphore = Arc::new(tokio::sync::Semaphore::new(2));
        while let Some(incoming) = rx.recv().await {
            let handler = Arc::clone(&self);
            let resp_tx = response_tx.clone();
            let is_delta = matches!(incoming.request, crate::network::SyncRequest::Delta { .. });
            if is_delta {
                let sem = Arc::clone(&delta_semaphore);
                ...
                match permit {
                    ...
                    Err(_) => {
                        debug!("Delta sync request dropped — concurrency limit reached");
                    }
                }
            } else {
                tokio::spawn(async move {
                        ...
                        if vote_state.0 >= vote_height && !(vote_state.0 == vote_height && vote_state.1 == request.checkpoint_hash) {
                            if vote_state.0 == vote_height {
                                warn!(
                                    ...
                                    &request.checkpoint_hash[..16.min(request.checkpoint_hash.len())],
                                    vote_height,
                                    &vote_state.1[..16.min(vote_state.1.len())]
                                );
                            } else {
                                debug!(
                                    "Declining stale vote request at height {} — already voted at height {}",
                                    vote_height, vote_state.0
                                );
                            }
                            VoteResponse::CheckpointVote(None)
                        } else {
    ...
    async fn process_sync_request(
        ...
        let response = match incoming.request {
            ...
            SyncRequest::Delta { from_checkpoint } => {
                ...
                let local_tip = self.state.get_checkpoint_height();
                if from_checkpoint > local_tip {
                    debug!(
                        "Delta sync short-circuit: peer at {} > our tip {}, nothing to send",
                        from_checkpoint, local_tip
                    );
                ...
                } else if from_checkpoint == local_tip {
                    const MAX_MEMPOOL_SYNC_TXS: usize = 200;
                    ...
                    for n in all_unfinalized.iter() {
                        ...
                        mempool_txs.push(TransactionData {
                            ...
                            nonce: n.tx.tx.nonce,
                            timestamp: n.tx.tx.timestamp,
                            signature: n.tx.signature.clone(),
                            parents: n.tx.tx.parents.clone(),
                            gas_price: n.tx.tx.gas_price.unwrap_or(0),
                            memo: n.tx.tx.memo.clone(),
                            references: n.tx.tx.references.clone(),
                        });
                    }
                    drop(state);
                    ...
                    if !mempool_txs.is_empty() {
                        info!(
                            "Delta mempool sync: serving {} unfinalized txs to peer at same height {} ({}ms)",
                            mempool_txs.len(), local_tip, delta_ms
                        );
                    }
                    SyncResponse::Delta(DeltaData {
            ...
            SyncRequest::CheckpointPush(push_data) => {
                ...
                let mut backfill_applied = 0u64;
                if !push_data.backfill_checkpoints.is_empty() {
                    let mut sorted_backfills = push_data.backfill_checkpoints;
                    sorted_backfills.sort_by_key(|b| b.checkpoint.height);
                    for bf in sorted_backfills {
                        let bf_height = bf.checkpoint.height;
                        let current = self.state.get_checkpoint_height();
                        if bf_height <= current {
                            continue;
                        }
                        ...
                        let applied = self
                            .apply_received_checkpoint(
                                bf.checkpoint,
                                bf.finalized_tx_hashes,
                                bf.finalized_transactions,
                                bf.precomputed_proofs,
                                "backfill-push",
                            )
                            .await;
                        if applied {
            ...
            if elapsed >= 10 {
                ...
                inner.fast_path_confirmed.retain(|tx_hash, finality| {
                    let entry_age = if let Some(confirmed_at) = finality.confirmed_at_ms {
                        ...
                        now_ms.saturating_sub(finality.registered_at_ms)
                    };
        ...
        'already_seen: {
            if checkpoint.height <= local_height {
                ...
                if source == "sync-push" && !finalized_transactions.is_empty() {
                    ...
                    return true;
                } else if source == "sync-push" && !precomputed_proofs.is_empty() {
                    info!(
                        "Late sync-push for already-applied checkpoint {} at height {} — retroactively adding {} proofs (no tx bodies)",
                        &checkpoint.hash[..16.min(checkpoint.hash.len())],
                        checkpoint.height,
                        precomputed_proofs.len()
                    );
    ...
    async fn apply_single_checkpoint_inner(
        ...
        let use_proof_verified = !precomputed_proofs.is_empty() && precomputed_proofs.len() >= 1;
        let apply_result = if use_proof_verified {
            info!(
                "PROOF-VERIFIED PATH: applying checkpoint {} at h={} via proof verification ({} proofs, skipping re-execution)",
                &checkpoint.hash[..16.min(checkpoint.hash.len())],
                checkpoint.height,
                precomputed_proofs.len()
            );
            ...
            match proof_result {
                ...
                Err(e) => {
                    warn!(
                        "PROOF-VERIFIED FALLBACK: proof verification failed for checkpoint {} at h={}, falling back to execution path: {}",
                        &checkpoint.hash[..16.min(checkpoint.hash.len())],
                        checkpoint.height,
                        e
                    );
        ...
        match apply_result {
            Err(e) => {
                warn!(
                    "Failed to apply checkpoint {} via {} (took {}ms): {}",
                    &checkpoint.hash[..16.min(checkpoint.hash.len())],
                    source,
                    apply_ms,
                    e
            ...
            Ok(missing_tx_count) => {
                ...
                info!(
                    "Applied checkpoint {} at height {} via {} (added {} txs from leader, apply={}ms, catching_up={})",
                    &checkpoint.hash[..16.min(checkpoint.hash.len())],
                    checkpoint.height,
                    source,
                    added_from_leader,
    ...
    pub async fn cleanup_finalized_full(&self, finalized_hashes: &[String], checkpoint_height: u64) {
        ...
        inner.stale_nonce_cache.clear();
        inner.stale_nonce_cache_order.clear();
        inner.leader_intents.remove(&checkpoint_height);
        inner.leader_timeout_votes.remove(&checkpoint_height);
    ...
    pub async fn try_lock_proposer_vote(&self, height: u64, checkpoint_hash: &str) -> Result<(), String> {
        ...
        if vote_state.0 == height && !vote_state.1.is_empty() && vote_state.1 != checkpoint_hash {
            return Err(format!(
                "already voted for {} at height {}",
                &vote_state.1[..16.min(vote_state.1.len())],
                height
            ));
        }
        *vote_state = (height, checkpoint_hash.to_string());
        Ok(())
    }

    pub async fn clear_vote_lock_for_height(&self, height: u64) {
        let mut vote_state = self.qcc_vote_lock.lock().await;
    ...
    pub async fn clear_leader_timeouts_up_to(&self, height: u64) {
        ...
        inner.view_change_votes.retain(|(h, _), _| *h > height);
        inner.view_change_sent.retain(|(h, _)| *h > height);
        inner.current_view.retain(|h, _| *h > height);
    }

    pub async fn broadcast_view_change(
        &self,
        height: u64,
        view: u32,
        validator_address: &str,
        ...
        let mut inner = self.inner.write().await;
        let key = (height, view);
        if inner.view_change_sent.contains(&key) {
            return;
        }
        inner.view_change_sent.insert(key);

        let bls_sig = if let Some(ref signer) = self.checkpoint_vote_signer {
        ...
        let message = GossipMessage::ViewChange {
            height,
            view,
            validator_address: validator_address.to_string(),
            validator_stake,
    ...
    fn verify_view_change_bls_signature(
        height: u64,
        view: u32,
        validator_address: &str,
        bls_signature: &str,
        ...
        let msg = Self::view_change_signing_message(height, view, validator_address);
        crate::bls::bls_verify(&msg, &sig_bytes, &pk_bytes)
    }

    pub async fn record_view_change_vote(
        &self,
        height: u64,
        view: u32,
        validator_address: &str,
    ...
    pub async fn get_view_change_certificate(
        &self,
        height: u64,
        view: u32,
    ) -> Option<rinku_core::types::ViewChangeCertificate> {
        let (_, _, _, total_stake, _) = self.get_cached_validators().await;
        let inner = self.inner.read().await;
        let key = (height, view);
        if let Some(votes) = inner.view_change_votes.get(&key) {
            let quorum_stake: u64 = votes.values().map(|v| v.validator_stake).sum();
            let quorum_threshold = (total_stake * 2 + 2) / 3;
            if quorum_stake >= quorum_threshold {
                Some(rinku_core::types::ViewChangeCertificate {
                    height,
                    view,
    ...
    pub async fn get_current_view(&self, height: u64) -> u32 {
        let inner = self.inner.read().await;
        inner.current_view.get(&height).copied().unwrap_or(0)
    }

    pub fn verify_view_change_certificate(
        cert: &rinku_core::types::ViewChangeCertificate,
        validators: &HashMap<String, rinku_core::types::Validator>,
    ) -> bool {
        ...
        accumulated_stake >= quorum_threshold
    }

    pub async fn cache_checkpoint_data(&self, height: u64, data: CachedCheckpointData) {
        let mut inner = self.inner.write().await;
    ...
    #[cfg(feature = "p2p")]
    pub fn push_checkpoint_to_peers(
        self: &Arc<Self>,
        push_data: crate::network::CheckpointPushData,
    ) {
        let gossip = Arc::clone(self);
        tokio::spawn(async move {
            let network_handle = match gossip.network_handle {
                Some(ref h) => h.clone(),
                None => return,
            };
                ...
                match network_handle.send_sync_request(peer_id, request).await {
                    Ok(rx) => {
                        match tokio::time::timeout(
                            ...
                            Ok(Ok(_)) => {
                                warn!(
                                    "SYNC-PUSH: peer {} unexpected response for checkpoint {} h={} — will retry",
                                    &peer_id[..12.min(peer_id.len())], cp_hash_prefix, cp_height
                                );
                                failed_peers.push(peer_id.clone());
                            }
                            Ok(Err(_)) => {
                                warn!(
                                    "SYNC-PUSH: peer {} channel closed for checkpoint {} h={} — will retry",
                                    &peer_id[..12.min(peer_id.len())], cp_hash_prefix, cp_height
                                );
                                failed_peers.push(peer_id.clone());
                            }
                            Err(_) => {
                                warn!(
                                    "SYNC-PUSH: peer {} timed out ({}ms) for checkpoint {} h={} — will retry",
                                    &peer_id[..12.min(peer_id.len())], push_timeout_ms, cp_hash_prefix, cp_height
                                );
                                failed_peers.push(peer_id.clone());
                            }
                        }
                    ...
                    Err(e) => {
                        warn!(
                            "SYNC-PUSH: failed to send to peer {} for checkpoint {} h={}: {} — will retry",
                            &peer_id[..12.min(peer_id.len())], cp_hash_prefix, cp_height, e
                        );
                        failed_peers.push(peer_id.clone());
                    }
                }
            ...
            if !failed_peers.is_empty() {
                ...
                let mut inner = gossip.inner.write().await;
                for peer_id in failed_peers {
                    inner.push_retry_queue.push(PendingCheckpointPush {
                        peer_id,
                        push_data: std::sync::Arc::clone(&shared_push_data),
                        attempts: 1,
                        last_attempt: now,
                        created_at: now,
                    });
                ...
                if inner.push_retry_queue.len() > MAX_RETRY_QUEUE_SIZE {
                    ...
                    if !dropped.is_empty() {
                        warn!(
                            "SYNC-PUSH-RETRY: evicted {} stale retry entries (lowest h={}), queue capped at {}",
                            dropped.len(),
                            dropped.first().map(|e| e.push_data.checkpoint.height).unwrap_or(0),
    ...
    #[cfg(feature = "p2p")]
    pub async fn drain_push_retries(&self) {
        const MAX_ATTEMPTS: u8 = 5;
        const RETRY_INTERVAL_MS: u64 = 500;
        const MAX_AGE_SECS: u64 = 30;

        let entries: Vec<PendingCheckpointPush> = {
            let mut inner = self.inner.write().await;
            std::mem::take(&mut inner.push_retry_queue)
        };
        ...
        let network_handle = match self.network_handle {
            Some(ref h) => h.clone(),
            None => return,
        };
        ...
        let local_height = self.state.get_checkpoint_height();
        let now = std::time::Instant::now();
        let mut requeue: Vec<PendingCheckpointPush> = Vec::new();

        for mut entry in entries {
            if entry.push_data.checkpoint.height < local_height.saturating_sub(2) {
                continue;
            }
            if entry.attempts >= MAX_ATTEMPTS {
                warn!(
                    "SYNC-PUSH-RETRY: giving up on peer {} for checkpoint h={} after {} attempts",
                    &entry.peer_id[..12.min(entry.peer_id.len())],
                    entry.push_data.checkpoint.height,
                    entry.attempts
                );
                continue;
            }
            ...
            if entry.last_attempt.elapsed().as_millis() < RETRY_INTERVAL_MS as u128 {
                requeue.push(entry);
                continue;
            }
        ...
        if !requeue.is_empty() {
            ...
            debug!("SYNC-PUSH-RETRY: {} entries re-queued", count);
        }
    }
    ...
    pub async fn drain_checkpoint_buffer(&self) {
        loop {
            ...
            match buffered {
                Some(b) => {
                    info!(
                        "Applying buffered checkpoint at height {} (source: {}, catch-up mode)",
                        next_height, b.source
                    );
                    let applied = self
                        .apply_single_checkpoint_catching_up(
                            b.checkpoint,
    ...
    pub async fn handle_message(&self, message: GossipMessage) -> Result<Option<GossipMessage>> {
        ...
        match message {
            ...
            GossipMessage::ViewChange {
                ...
                if height != target_height {
                    debug!(
                        "Ignored ViewChange for height {} (target={})",
                        height, target_height
                    );
                    return Ok(None);
                }

                let (validators, our_addr, our_stake, _total_stake, _quorum) =
                    self.get_cached_validators().await;
                if !validators.contains_key(&validator_address) {
                    warn!(
                ...
                let validator = validators.get(&validator_address).unwrap();
...

                {
                    ...
                    if vote_state.0 >= height && !(vote_state.0 == height && vote_state.1 == checkpoint_hash) {
                        if vote_state.0 == height {
                            ...
                            debug!(
                                "Declining stale gossip vote request at height {} — already voted at height {}",
                                height, vote_state.0
                            );
                        }
                        return Ok(None);
                    }
                    *vote_state = (height, checkpoint_hash.clone());
                }
    ...
    pub async fn get_peer_health_summary(&self) -> String {
        let inner = self.inner.read().await;
    ...
    pub async fn record_peer_sync_failure(&self, peer_id_prefix: &str) {
        let mut inner = self.inner.write().await;
        for peer_info in inner.peers.values_mut() {
            if peer_info.node_id.contains(peer_id_prefix) {
                peer_info.consecutive_failures += 1;
                if peer_info.consecutive_failures >= 5 && peer_info.is_healthy {
                    peer_info.is_healthy = false;
                    tracing::warn!(
                        "PEER-HEALTH: Marking peer {} degraded after {} consecutive sync failures",
                        &peer_info.node_id[..16.min(peer_info.node_id.len())],
                        peer_info.consecutive_failures
                    );
                }
                break;
            }
    ...
    pub async fn record_peer_sync_success(&self, peer_id_prefix: &str) {
        let mut inner = self.inner.write().await;
        for peer_info in inner.peers.values_mut() {
            if peer_info.node_id.contains(peer_id_prefix) {
                if peer_info.consecutive_failures > 0 {
                    tracing::info!(
                        "PEER-HEALTH: Peer {} recovered (was at {} failures)",
                        &peer_info.node_id[..16.min(peer_info.node_id.len())],
                        peer_info.consecutive_failures
                    );
    ...
    fn extract_sender_url(message: &GossipMessage) -> Option<String> {
        match message {
            ...
            GossipMessage::ViewChange { sender_url, .. } => sender_url.clone(),
