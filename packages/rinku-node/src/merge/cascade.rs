...
pub fn cascade_rollback(
    ...
    let surviving_txs: Vec<&PartitionTxSummary> = all_txs.iter()
        .filter(|tx| !losing_tx_hashes.contains(&tx.tx_hash))
        .collect();

    let mut ordered_txs = surviving_txs.clone();
    ...
    loop {
        ...
        if newly_rolled_back.is_empty() {
            let cascade_details: Vec<CascadeRollback> = rolled_back.iter()
                .filter(|h| !losing_tx_hashes.contains(*h))
                .filter_map(|h| {
                    let tx = all_txs.iter().find(|t| t.tx_hash == *h)?;
                    let reason = rollback_reasons.get(h).cloned()
            ...
            let surviving_hashes: Vec<String> = ordered_txs.iter()
...
mod tests {
    ...
    fn test_nonce_gap_cascade() {
        ...
        assert!(matches!(report.cascade_rollbacks[0].reason, RollbackReason::NonceContinuityGap));
        assert_eq!(report.surviving_tx_hashes, vec!["tx1"]);
    }
