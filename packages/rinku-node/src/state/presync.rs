...
#[cfg(feature = "p2p")]
use crate::network::{PeerHandshake, SnapshotData, SyncRequest, SyncResponse};
...
#[cfg(feature = "p2p")]
pub(crate) async fn try_presync_from_peers(
    ...
    if bootstrap_peers.is_empty() {
        if is_genesis_node {
            info!(
                "No bootstrap peers configured, will create genesis locally (IS_GENESIS_NODE=true)"
            );
        } else {
    ...
    for attempt in 1..=max_retries {
        info!(
            "PRE-SYNC: Attempt {}/{} - syncing from {} bootstrap peer(s)...",
            attempt,
            max_retries,
            bootstrap_peers.len()
        );
        ...
        if attempt < max_retries {
            ...
            info!(
                "PRE-SYNC: Attempt {} failed, retrying in {}s...",
                attempt, delay
            );
    ...
    if !is_genesis_node {
        warn!(
            "PRE-SYNC: All {} attempts failed! Validator node cannot create genesis.",
            max_retries
        );
    } else {
        ...
        match peer_str.parse::<Multiaddr>() {
            Ok(addr) => {
                ...
                if let Some(ref pid) = peer_id {
                    ...
                    info!(
                        "PRE-SYNC: Parsed address {} (peer ID will be discovered)",
                        dial_addr
                    );
                }
...
pub fn convert_snapshot_data_to_sync_snapshot(data: SnapshotData) -> SyncSnapshot {
    use rinku_core::types::{Account, Checkpoint, SignedTransaction, Transaction, Validator};
    ...
    let genesis_hash = data
        .checkpoints
        .first()
        .and_then(|c| c.genesis_hash.clone());

    // Convert accounts
    let accounts: HashMap<String, Account> = data
        .accounts
        .into_iter()
        .map(|a| {
            (
    ...
    // Convert validators
    let validators: HashMap<String, Validator> = data
        .validators
        .into_iter()
        .map(|v| {
            (
    ...
    // Convert checkpoints
    let checkpoints: Vec<Checkpoint> = data
        .checkpoints
        .iter()
        .enumerate()
        .map(|(i, c)| {
            ...
            Checkpoint {
                ...
                hash: c
                    .hash
                    .clone()
                    .unwrap_or_else(|| rinku_core::sha256_hex(&format!("cp:{}", c.height))),
    ...
    let dag_transactions: Vec<SignedTransaction> = data
        .recent_txs
        .into_iter()
