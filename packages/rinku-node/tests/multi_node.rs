...
use rinku_core::types::{Account, Checkpoint, SignedTransaction, Transaction, Validator, ValidatorSignature, DagNode};
...
fn create_test_transaction(from: &str, to: &str, amount: f64, nonce: u64, parents: Vec<String>) -> SignedTransaction {
    let tx = Transaction {
        from: from.to_string(),
        to: to.to_string(),
        amount,
...
fn create_test_checkpoint(height: u64, previous_hash: Option<String>, tx_hashes: &[&str]) -> Checkpoint {
...
impl SimulatedNode {
    fn new(id: &str) -> Self {
        ...
        accounts.insert("genesis".to_string(), create_test_account("genesis", 1_000_000.0, 0));
    ...
    fn add_transaction(&mut self, tx: SignedTransaction) {
        ...
        let from_balance = self.accounts.get(&tx.tx.from).map(|a| a.balance).unwrap_or(0.0);
        
        if from_balance >= tx.tx.amount + fee {
            let from_acc = self.accounts.entry(tx.tx.from.clone()).or_insert(
                create_test_account(&tx.tx.from, 0.0, 0)
            );
            from_acc.balance -= tx.tx.amount + fee;
            from_acc.nonce = tx.tx.nonce;
            
            let to_acc = self.accounts.entry(tx.tx.to.clone()).or_insert(
                create_test_account(&tx.tx.to, 0.0, 0)
            );
    ...
    fn create_checkpoint(&mut self) -> Checkpoint {
        ...
        let previous_hash = self.checkpoints.last()
            .map(|c| c.hash.clone());
...
mod fork_detection_tests {
    ...
    fn test_fork_detection_via_previous_hash_mismatch() {
        ...
        assert_ne!(cp1.hash, cp2.hash, "Checkpoint hashes should differ (fork detected)");
        assert_eq!(cp1.previous_hash, cp2.previous_hash, "Previous hashes should match (common ancestor)");
    }
    ...
    fn test_consecutive_previous_hash_mismatches_triggers_recovery() {
        ...
        for i in 2..=4 {
            let tx_node1 = create_test_transaction("genesis", "bob", 10.0 * i as f64, i as u64, vec![]);
            let tx_node2 = create_test_transaction("genesis", "charlie", 10.0 * i as f64, i as u64, vec![]);
...
mod snapshot_sync_tests {
    ...
    fn test_snapshot_contains_all_accounts() {
        ...
        node.add_transaction(create_test_transaction("genesis", "alice", 100.0, 1, vec![]));
        node.add_transaction(create_test_transaction("genesis", "bob", 200.0, 2, vec![]));
        node.add_transaction(create_test_transaction("genesis", "charlie", 300.0, 3, vec![]));
    ...
    fn test_snapshot_sync_restores_balances() {
        ...
        source_node.add_transaction(create_test_transaction("genesis", "alice", 100.0, 1, vec![]));
...
mod checkpoint_adoption_tests {
    ...
    fn test_adopt_peer_checkpoint_with_matching_transactions() {
        ...
        assert!(can_adopt, "Should be able to adopt checkpoint with matching transactions");
    ...
    fn test_reject_peer_checkpoint_with_different_transactions() {
        ...
        node1.add_transaction(create_test_transaction("genesis", "alice", 100.0, 1, vec![]));
        ...
        assert!(!can_adopt, "Should NOT adopt checkpoint with different transactions");
    }
    ...
    fn test_checkpoint_chain_linkage_validation() {
        ...
        assert!(!can_adopt_orphan, "Should reject checkpoint with wrong previous_hash");
    }
...
mod fork_resolution_tests {
    ...
    fn test_dag_structure_and_tips() {
        ...
        let branch_b = create_test_transaction("alice", "charlie", 50.0, 1, vec![genesis.hash.clone()]);
        let branch_b_node = create_dag_node(branch_b.clone(), 1.0);
        dag.add_node(branch_b_node).unwrap();
        
        assert_eq!(dag.tip_count(), 2, "Should have 2 tips (two branches from genesis)");
    ...
    fn test_conflicting_nonces_detected() {
        ...
        assert!(has_conflict, "Same sender + same nonce = double-spend attempt");
    }
    ...
    fn test_higher_weight_branch_wins() {
        ...
        let winner = if branch_a_weight > branch_b_weight { "A" } else { "B" };
    ...
    fn test_weight_comparison_logic() {
        ...
        let branch_a = Branch { confirmations: 5, stake_weight: 1.0 };
        let branch_b = Branch { confirmations: 2, stake_weight: 1.0 };
        ...
        let branch_c = Branch { confirmations: 2, stake_weight: 3.0 };
        let branch_d = Branch { confirmations: 5, stake_weight: 1.0 };
...
mod delta_sync_tests {
    ...
    fn test_delta_sync_fetches_missing_transactions() {
        ...
        let delta_txs: Vec<SignedTransaction> = source_node.dag.iter()
    ...
    fn test_paginated_delta_sync() {
        ...
        for page in 0..pages_needed {
            ...
            let batch: Vec<SignedTransaction> = source_node.dag
                .iter()
                .skip(offset)
                .take(page_size)
                .cloned()
        ...
        assert_eq!(fetched_txs.len(), 100, "Should have fetched all transactions");
    }
}
...
mod network_partition_tests {
    ...
    fn test_partition_creates_divergent_chains() {
        ...
        for i in 2..=5 {
            let tx_a = create_test_transaction("genesis", &format!("user_a_{}", i), 10.0, i, vec![]);
            let tx_b = create_test_transaction("genesis", &format!("user_b_{}", i), 10.0, i, vec![]);
    ...
    fn test_partition_heal_requires_fork_resolution() {
        let mut nodes = vec![
            SimulatedNode::new("node1"),
            SimulatedNode::new("node2"),
        ];
        ...
        assert!(fork_detected, "Fork should be detected when partitions heal");
