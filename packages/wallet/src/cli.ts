...
commands:
  ...
  faucet <address>       request RKU from faucet
  send <from-key> <to> <amount>  send RKU
...
`);
  ...
  if (command === "faucet") {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        console.log("received:", data.amount, "RKU");
  ...
  if (command === "send") {
    ...
    try {
      ...
      console.log("\nsending", amount, "RKU to", to.slice(0, 8) + "...");

      const { tx, url } = await wallet.send(to, amount);
      const pubkeyB64 = Buffer.from(wallet.getPublicKey()).toString(
        "base64url",
      );
  ...
  if (command === "dag") {
    try {
      ...
      data.nodes.forEach((node, i) => {
        ...
        console.log(
          `      ${node.tx.from === "genesis" ? "genesis" : node.tx.from.slice(0, 8) + "..."} → ${node.tx.to.slice(0, 8)}...`,
        );
