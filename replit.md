...
- **ZK Privacy Layer (In Development):** Optional privacy-preserving proofs using Groth16 ZK-SNARKs. Enables `rinku://zk/{payload}` URLs that prove transaction validity without revealing sender, recipient, or amount. Uses Poseidon hashing (ZK-friendly), Pedersen commitments for hidden amounts, EdDSA signatures in-circuit, and nullifiers to prevent double-claims. Base chain stays transparent; privacy is opt-in at the proof layer. ~500 char QR-compatible proofs with 10ms verification. Implementation: `packages/zk/` workspace with Circom circuits and TypeScript infrastructure. Node API: `/api/zk/status`, `/api/zk/witness/:txHash`, `/api/zk/verify`. Explorer: ZK Privacy tab for witness generation and proof verification. See Appendix H in WHITEPAPER.md for full specification.
