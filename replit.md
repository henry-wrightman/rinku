...
- **Self-Contained Proof System (v4 - MerkleSumTree):** Fully offline-verifiable transaction proofs with zero trust assumptions beyond cryptography. Each proof contains: transaction data, checkpoint header (txMerkleRoot, stateRoot, receiptRoot, tipCount), Merkle inclusion proof, BLS aggregated signature, MerkleSumTree multi-proof (batched signer membership proofs with shared siblings), and validatorSumTreeRoot (hash + totalWeight). Verification derives totalWeight from the MerkleSumTree root (cryptographically bound), recomputes signer weight from membership proofs, and verifies 67% weight threshold. Closes denominator attack vulnerability - totalWeight cannot be forged. Multi-proof optimization achieves 60-75% size reduction by sharing sibling nodes. Format: `rinku://sp/{base64url-deflate-json}`. QR-compatible with packed encoding + multi-proof for committees N ≤ 21.
