...
- **Protocol-Level Tip Consolidation:** Automatic DAG tip reduction to prevent O(n) CPU scaling with tip count. Staked validators create zero-fee consolidation transactions (`kind: 'consolidation'`) that reference multiple tips to collapse branches. Configuration: upper threshold (200 tips triggers consolidation), lower threshold (100 tips stops consolidation, hysteresis), tips per consolidation (32), interval (3s), cooldown (5s). API endpoint: `/api/tip-consolidator/stats`. Prometheus metrics: `rinku_tip_consolidations_total`, `rinku_tips_consolidated_total`. Consolidation txs bypass gas fees but still require valid validator signatures and nonces.
